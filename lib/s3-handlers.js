...
module.exports = function(config, logger) {
  ...
  function buildS3Params(req) {
    ...
    const s3key = config.s3.prefix + '/' + id + '/' + filename;
    const params = {
      Bucket: s3bucket,
      Key: s3key,
    };
  ...
  async function uploadHandler(req, res, next) {
    ...
    job.on('success', async () => {
      try {
        await Promise.all([
          uploadToS3(job.getCmdPath(), config.s3.bucket, getJobCmdS3Key(job.id)),
          uploadToS3(job.getResultPath(), config.s3.bucket, getJobResultS3Key(job.id)),
          uploadToS3(job.getStdoutPath(), config.s3.bucket, getJobStdoutS3Key(job.id)),
          uploadToS3(job.getStderrPath(), config.s3.bucket, getJobStderrS3Key(job.id)),
        ]);
      } catch(e) {
